...
body {font-family: Arial, sans-serif; margin: 0;background-image: url('https://t3.ftcdn.net/jpg/04/26/06/70/240_F_426067028_WKg5UlRSdxTgFwkeqMvawemzhC5YywCL.jpg'); background-repeat: no-repeat; background-size:  100% 100%; background-position: inherit;}
...
<h1 style="text-align: center; font-size: 40px; color: white;">LIST ALL BATCH </h1>
<table id="customers" class="w3-table-all w3-xxxlarge" border="1" style="background:azure;">
	...
	<c:forEach var="batch" items="${batchs}">
		<tr>
			...
			<td>${batch.getTrainers()}</td>
		
			<td id="delete"><a  href="deleteBatch?Id=${batch.getId()}">Delete</a></td>
			<td id="edit" ><a href="updateBatch?Id=${batch.getId()}">Edit</a></td>
		</tr>
	</c:forEach>
</table>
